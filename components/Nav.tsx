  ...
  return (
    <nav style={{
            ...
            {accountMenuOpen && (
              <div style={{ position: "absolute", top: "calc(100% + 10px)", right: 0, background: "#16140f", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "10px", minWidth: "210px", overflow: "hidden", zIndex: 160, boxShadow: "0 12px 32px rgba(0,0,0,0.5)" }}>
                ...
                {[
                  ...
                  { label: "My experiences", href: `/profile/${user.id}` },
                ].map((item) => (
                  <Link
                    key={item.label}
                    href={item.href}
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "11px 16px", fontFamily: "'Inter', sans-serif", fontSize: "13px", color: "rgba(246,241,234,0.45)", cursor: "default", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                  Notifications
      ...
      <div className="mobile-nav" style={{ display: "flex", alignItems: "center", gap: "12px" }}>
        {user && (
          <Link href={`/profile/${user.id}`} aria-label="Your profile" style={{ display: "flex", alignItems: "center" }}>
